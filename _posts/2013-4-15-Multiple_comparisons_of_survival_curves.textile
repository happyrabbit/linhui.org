...
Here is a brief description of how the Sidak’s adjustment works. If we are concerned about all k comparisons, we might be more interested in the probability that we reject a true null hypothesis at all. That is,
...
The Sidak’s formula is to use α = 1 − (1−α_t)/k given a desired α_t. In the above situation, if we wanted  α_t= 0.05, this formula would show us that we need to use an α=0.00256.
