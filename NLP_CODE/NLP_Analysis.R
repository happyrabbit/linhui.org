...
load("annotres1.RData")
load("annotres2.RData")
load("annotres3.RData")
load("annotres4.RData")
load("annotres5.RData")
str(annotres1)

unique(token1$id )
table(annotres1$token$POS)


senti1<-data.frame(annotres1$sentiment)
#senti[12,]
...
idx<-which(token1$POS=="RB")
idx<-which(token1$POS=="NNP")

words<-token[idx,]$token
table(words)
