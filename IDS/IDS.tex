...
\begin{verbatim}
## 124 196 238 284 440 477 482 594 704 765 
##   0   0   1   0   0   1   0   1   1   1 
## Levels: 0 1
\end{verbatim}
...
You can calculate the total accuracy when there are more than two categories. This statistic is straightforward but has some disadvantages. First, it doesn't differentiate different error types. In a real application, different types of error may have different impacts. For example, it is much worse to tag an important email as spam and miss it than failing to filter out a spam email. Provost et al. \citep{Provost1998} discussed in detail the problem of using total accuracy on different classifiers. There are some other metrics based on the confusion matrix that measure different types of error. P\textbf{recision} is a metric to measure how accurate positive predictions are (i.e.~among those emails predicted as spam, how many percentages of them are spam emails?):

\[precision = \frac{TP}{TP+FP}\]

\textbf{Recall sensitivity} is to measure the coverage of actual positive samples (i.e.~among those spam emails, how many percentages of them are predicted as spam) :
